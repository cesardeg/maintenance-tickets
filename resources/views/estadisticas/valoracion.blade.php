	...
	<div class="container-fluid">
		<div class="card">
			<div class="card-header">
				<h3 class="card-title">Estadistícas por tiempo de valoración</h3>
            </div>
			<!-- /.card-header -->
			<div class="card-body">
                ...
                <div class="row">
                    <div class="col">
                        <h2>Promedio de días en hacer una visita:
                            @if($data[0]->total != null)
                            <span id="total-fixes">{{ $data[0]->total }}</span>
                            @else
                            <span id="total-fixes">Sin visitas</span>
                            @endif
                        </h2>
                    </div>
                </div>
...
<script>
    ...
    function obtenerData() {
        ...
        $.ajax({
            url: '/estadistica-valoracion/getData',
    ...
    function updateTotalTime(total){
        if(total == null){
            document.querySelector('#total-fixes').textContent = "Sin visitas"
        }else{
